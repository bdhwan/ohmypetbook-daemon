...
const COMMAND_HANDLERS = {
  get_versions: async () => {
    const daemonCurrent = DAEMON_VERSION;
    const openclawCurrent = getOpenclawVersion();
    let daemonLatest = '', openclawLatest = '';
    try {
      const [dResp, oResp] = await Promise.all([
        fetch('https://registry.npmjs.org/ohmypetbook/latest').catch(() => null),
        fetch('https://registry.npmjs.org/openclaw/latest').catch(() => null),
      ]);
      if (dResp?.ok) daemonLatest = (await dResp.json()).version || '';
      if (oResp?.ok) openclawLatest = (await oResp.json()).version || '';
    } catch {}
    return {
      daemonVersion: daemonCurrent,
      daemonLatest,
      daemonUpdateAvailable: !!(daemonLatest && daemonCurrent && daemonLatest !== daemonCurrent),
      openclawVersion: openclawCurrent,
      openclawLatest,
      openclawUpdateAvailable: !!(openclawLatest && openclawCurrent && openclawLatest !== openclawCurrent),
      hasOpenclaw: hasOpenclaw(),
    };
  },

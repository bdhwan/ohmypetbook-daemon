    ...
    try {
      ...
      if (npmBin) {
        ...
        if (fs.existsSync(npmPkgPath)) {
          method = 'npm';
          log('📦 npm update 실행 중...');
          _execSync(`${npmBin} update -g ohmypetbook`, { encoding: 'utf-8', timeout: 60000, env });
    ...
    if (hasOpenclaw() && getIdTokenCallback) {
      try {
        ...
        if (Object.keys(envKeys).length > 0) {
          ...
          const existing = petSnap.exists() ? (petSnap.data().envSecrets || {}) : {};
          ...
          if (Object.keys(newSecrets).length > 0) {
            data.envSecrets = { ...existing, ...newSecrets };

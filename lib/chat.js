    ...
    this.ws.on("message", (data) => {
      try {
        ...
        if (msg.type === "res") {
          ...
          if (msg.event === "chat") {
            log(`📡 chat event: state=${msg.payload?.state} sk=${msg.payload?.sessionKey} runId=${msg.payload?.runId?.slice(0,8)}`);
          } else if (msg.event !== "health" && msg.event !== "tick" && msg.event !== "agent") {
          ...
          try {
            ...
            const streamPromise = new Promise((resolve, reject) => {
              ...
              function chatHandler(payload) {
                if (!payload) return;
                log(`🔍 chat event: state=${payload.state} sk=${payload.sessionKey} expected=${sessionKey}`);

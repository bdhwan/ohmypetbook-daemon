...
async function handleBotRequest(db, groupId, reqData, reqRef) {
  ...
  try {
    ...
    if (!reqData.shouldRespond) {
      ...
      // chat.inject 시도 → 세션 없으면 chat.send로 세션 생성 후 재시도
      try {
        await gw.inject(sessionKey, contextMsg, reqData.firestoreDocId);
      } catch (e) {
        if (e.message?.includes("session not found") || e.message?.includes("INVALID_REQUEST")) {
          log(`🔧 [${reqData.targetPetName}] 세션 없음 → chat.send로 세션 초기화`);
          // __INIT__ 메시지로 세션 생성 (응답은 버림)
          await gw.send(
            sessionKey,
            `[SYSTEM] 그룹 채팅 세션 초기화 — groupId: ${groupId}`,
            `${reqData.firestoreDocId}-init`,
            null
          );
          // 세션 생성 후 inject 재시도
          await gw.inject(sessionKey, contextMsg, reqData.firestoreDocId);

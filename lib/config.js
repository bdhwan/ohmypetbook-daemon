...
export const PID_FILE = path.join(PETBOOK_HOME, "daemon.pid");
...
function resolveOpenclawPath() {
  ...
  if (fs.existsSync(path.join(defaultPath, "openclaw.json"))) return defaultPath;
  // 4. OPENCLAW_HOME 환경변수가 없어도 openclaw.json을 흔한 경로에서 탐색
  const altPaths = [
    path.join(HOME, ".openclaw"),
    "/etc/openclaw",
  ];
  for (const p of altPaths) {
    if (p !== defaultPath && fs.existsSync(path.join(p, "openclaw.json"))) return p;
  }
  return defaultPath;
}
...
function detectOpenclaw() {
  ...
  // findBin은 순환 import 방지를 위해 직접 탐색
  try {
    const nodeDir = path.dirname(process.execPath);
    if (fs.existsSync(path.join(nodeDir, "openclaw"))) return true;
  } catch {}
  try {
